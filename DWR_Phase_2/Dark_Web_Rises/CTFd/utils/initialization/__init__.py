...
def init_template_globals(app):
    ...
    # --- Dark Web Rises additions ---------------------------------------
    # scoreboard.html swaps to a "CTF has concluded" panel once the event is
    # over. CTFd has no `ctf` template global and Session exposes only
    # id/nonce/hash, so the original `ctf.end < Session.timestamp` check was
    # silently undefined -- Jinja treats undefined as falsy, so the panel
    # never appeared and nothing errored. Expose the real helper instead.
    app.jinja_env.globals.update(ctf_ended=ctf_ended)

    # Where the concluded panel points. Set FINAL_LEADERBOARD_URL in the
    # environment (see docker-compose.yml). Left unset, the panel renders
    # without a button rather than linking somewhere that doesn't exist.
    app.jinja_env.globals.update(
        final_leaderboard_url=os.getenv("FINAL_LEADERBOARD_URL", "").strip()
    )
