...
// The leaderboard panel is built as an HTML string, so anything interpolated
// into it has to be escaped. Team names come from server configuration rather
// than from players, but a name with an apostrophe or an angle bracket would
// still break the markup.
...
game.on_game_over = (teamScore, teamRank, top3Scores, qualified, phase2Url, phase2Username, phase2Password) => {
    if (currentTimerInterval) clearInterval(currentTimerInterval);
    ...
    // Phase 2 handoff. Only the top 50% of the final standings advance, and
    // the server only sends the link to those teams.
    if (qualified === true) {
        html += `<hr style='border-color: var(--border-color);'>`;
        html += `<h4 style='color: var(--accent-color);'>Phase 2 Access Granted</h4>`;
        html += `<p>Your team finished in the top 50% and is through to Phase 2.</p>`;
        // Credentials for the CTFd account created for this team. The server
        // only sends these to the team they belong to. escapeHtml: the team
        // name comes from configuration, and the password is generated, but
        // neither belongs in innerHTML unescaped.
        if (phase2Password) {
            html += `<div style='border: 1px solid var(--border-color); padding: 12px; margin: 12px 0;'>`;
            html += `<p style='margin: 0 0 8px 0; font-size: 13px; text-transform: uppercase; letter-spacing: 2px;'>Your Round 2 Login</p>`;
            html += `<p style='margin: 4px 0;'>Username: <strong>${escapeHtml(phase2Username || "")}</strong></p>`;
            html += `<p style='margin: 4px 0;'>Password: <strong style='color: var(--accent-color); letter-spacing: 1px;'>${escapeHtml(phase2Password)}</strong></p>`;
            html += `<p style='margin: 8px 0 0 0; font-size: 12px;'>Write this down now &mdash; it is shown once, on this screen only.</p>`;
            html += `</div>`;
        }
        if (phase2Url) {
            const safeUrl = encodeURI(phase2Url);
            html += `<p><a href="${safeUrl}" target="_blank" rel="noreferrer" style='color: var(--accent-color);'>Enter Phase 2 &rarr;</a></p>`;
            html += `<p style='font-size: 12px; word-break: break-all;'>${safeUrl}</p>`;
        } else {
            html += `<p>Phase 2 link is not configured. Ask an event organiser for the URL.</p>`;
        }
    } else if (qualified === false) {
        html += `<hr style='border-color: var(--border-color);'>`;
        html += `<h4>Phase 2 Access Denied</h4>`;
        html += `<p>Only the top 50% of teams advance. Your run ends here &mdash; thanks for playing.</p>`;
    }
