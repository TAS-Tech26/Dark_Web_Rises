      ...
      <main className="relative mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="relative">
          ...
          {/* Server-side errors were previously rendered ONLY on the login
              screen, so a rejected prompt -- not your turn, rate limited,
              generation failed -- produced no message anywhere on the one
              screen where the player can actually act on it. */}
          {game.socketError && (
            <Panel className="mb-6 border-magenta/60 p-4" glow="magenta">
          ...
          {/* Main split */}
          {/* Below lg this stacks into one column in DOM order. The action
              column is given order-1 there so the prompt editor sits directly
              under the image instead of below the Active Player panel and the
              Turn Rules list -- on a phone that was roughly two screens of
              scrolling before the player could type, out of a 90s turn. */}
          <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr]">
            ...
            {/* Right — action */}
            <div className="flex flex-col gap-4">
              {/* order-last below lg: on a phone this panel and the Turn Rules
                  list sat between the image and the prompt editor, so the
                  player scrolled past both to reach the textarea. Reference
                  material belongs below the thing you act with. */}
              <Panel glow="neon" className="order-last p-5 lg:order-none">
              ...
              {!game.isMyTurn && game.promptPhase !== "accepted" && (
                <Panel variant="elevated" className="flex flex-col items-center gap-4 p-10 text-center">
                  ...
                  <div className="font-mono text-xs text-muted-foreground">
                    Your screen will switch automatically when it is your turn.
                  </div>
                </Panel>
              )}
              ...
              {game.promptPhase === "accepted" && (
                <Panel variant="elevated" className="p-5" glow="magenta">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Your prompt is being rendered. The next player takes over automatically —
                    the generated image will appear on their device.
                  </p>
                </Panel>
              )}
