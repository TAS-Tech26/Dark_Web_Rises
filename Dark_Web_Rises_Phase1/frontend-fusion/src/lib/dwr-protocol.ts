...
export const JSONFields = {
  ...
  /** Phase 2 handoff, sent with GAME_OVER. `qualified` is per team (top 50%
   * of the final standings); `phase2_url` is only populated when qualified. */
  QUALIFIED: "qualified",
  PHASE2_URL: "phase2_url",
  /** Generated CTFd credentials, matching the team's row in the import CSV.
   * Only ever sent to the team they belong to. */
...
export const Login = {
  ...
  LOCKED: "locked",
  /** Password was right, but the account is signed in on a live device.
   * Distinct from DENIED: retyping the password cannot fix it, and trying
   * will trip the brute-force lockout. */
  ALREADY_CONNECTED: "already_connected",
  /** Automatic reply to Responses.SESSION_PROBE. Never shown to the user. */
...
export const Responses = {
  ...
  /** "Is anyone still there?" — sent when a login is attempted for this
   * account elsewhere. Answer promptly with Login.SESSION_PROBE_ACK or the
   * server assumes this device is gone and releases the account. */
  SESSION_PROBE: "session_probe",
  /** This session was retired because it failed to answer a probe and the
   * account was claimed elsewhere. The user did not do this, so say so. */
