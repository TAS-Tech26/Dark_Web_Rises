...
# Models measured and rejected, with the reason, so the log line says WHY.
# Keyed by the exact DEEPINFRA_MODEL string.
REJECTED_MODELS = {
    "black-forest-labs/FLUX-2-klein-4b":
        "DeepInfra serves it serially -- 200 concurrent requests completed 0.49s "
        "apart, effective concurrency 0.85, so a 175-team round needs ~66s of a "
        "90s turn. Success rate and error rate both look perfect.",
}
...
class DeepInfraProvider(ImageProvider):
    ...
    def __init__(self, timeout, max_bytes):
        ...
        # Default is schnell, NOT klein-4b. klein-4b produces better images and
        # looks perfectly healthy under load -- 200 concurrent requests, zero
        # errors, zero 429s -- but DeepInfra serves it SERIALLY: 200 requests
        # took 98.3s, 0.49s apart, an effective concurrency of 0.85. Inference
        # is 420ms, so 99.2% of that is provider-side queueing. A 175-team round
        # needs 66s to drain against a 90s turn, with no margin.
        #
        # No conventional metric shows this; only the shape of the completion
        # timeline does (tools/analyse_run.py). That is exactly why the DEFAULT
        # matters: any deployment that does not carry .env verbatim -- env
        # injection, a container started with only the API key, startup.sh --
        # would otherwise silently run the unusable model and fail invisibly.
        self.model = os.getenv("DEEPINFRA_MODEL", "black-forest-labs/FLUX-1-schnell").strip()
        if self.model in REJECTED_MODELS:
            logger.error(
                "DEEPINFRA_MODEL=%r is a REJECTED model: %s. It will appear healthy "
                "(100%% success, no 429s) while silently exhausting the round clock. "
                "Set DEEPINFRA_MODEL=black-forest-labs/FLUX-1-schnell.",
                self.model, REJECTED_MODELS[self.model],
            )
        ...
        # Parsed defensively. A bare `DEEPINFRA_SIZE=` (an easy misreading of
        # "set to 0 to omit") makes int() raise ValueError, which escapes
        # build_chain on first use; the guard in Team.run_round then catches it
        # and scores the team 0. That failure is total and silent -- every team,
        # every round -- so an unparseable value falls back rather than raising.
        _raw_size = os.getenv("DEEPINFRA_SIZE", "512").strip()
        try:
            self.size = int(_raw_size) if _raw_size else 512
        except ValueError:
            logger.error(
                "DEEPINFRA_SIZE=%r is not an integer; falling back to 512. "
                "Set it to 0 to omit width/height entirely.", _raw_size,
            )
            self.size = 512
...
class ProviderChain:
    ...
    async def generate(self, client: httpx.AsyncClient, prompt: str) -> bytes | None:
        ...
        for provider in candidates:
            ...
            try:
                # httpx timeouts are PER-OPERATION, not per-request, so the
                # documented budget was never actually enforced: DeepInfra spends
                # it twice (POST, then _download of a returned URL) and Replicate
                # three or four times (its deadline is checked before issuing each
                # poll, so it overruns by a full timeout, then downloads). A
                # provider that trickles one chunk every 19s never trips anything
                # at all. Worst case for a two-provider chain was ~72s against a
                # 90s turn -- and unbounded in the trickle case.
                #
                # This is the outer ceiling the rest of the file assumes exists.
                # The per-call httpx timeouts stay as inner sub-budgets.
                ...
                # Explicit clause so a budget overrun reads as a timeout rather
                # than "raised unexpectedly". Counts against the breaker exactly
                # like any other failure, and falls through to the next provider.
                provider.breaker.record_failure()
                logger.warning(
                    "Provider %s exceeded its %.1fs budget (elapsed %.1fs); "
                    "failing over. %s",
                    provider.name, provider.timeout, time.perf_counter() - started, exc,
                )
                continue
...
def build_chain(timeout_primary=None, timeout_secondary=None, max_bytes=None) -> ProviderChain:
    ...
    # "One provider configured" and "a backup key is missing" produce the same
    # single-element chain but are very different states, and only one of them
    # is a decision. Per-request failover is the whole design here; without a
    # second provider, five consecutive failures -- which at 175 concurrent is
    # essentially instantaneous during any blip -- open the breaker and EVERY
    # request returns nothing for BREAKER_COOLDOWN_SECONDS. That is one full
    # turn boundary, up to 175 players, losing their turn per blip.
    if len(providers) == 1:
        logger.error(
            "Image provider chain has NO FALLBACK (%s only). A breaker trip means "
            "every request fails until the cooldown expires. Configure a second "
            "provider in IMAGE_PROVIDERS with a funded key, or accept this "
            "deliberately.",
            providers[0].name,
        )
