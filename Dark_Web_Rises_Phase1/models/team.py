...
# How long a send failure stays "recent" for the consecutive-failure count.
#
# Without this the counter is consecutive in SEQUENCE but not in TIME: it is
# only ever cleared by a successful send, so three timeouts at minute 2, minute
# 9 and minute 17 of a 40-minute game read as three in a row and evict a player
# who was fine the whole time. Intermittent congestion is exactly what the
# consecutive-failure rule was meant to tolerate.
SEND_FAILURE_DECAY_SECONDS = 30.0

# How many failed generations a player may absorb in one turn before the turn
# is forfeited. Counted separately from MAX_PROMPT_ATTEMPTS (which counts
# rejected *prompts*) because the two failures are not the player's in the same
# way -- but it is still bounded, so a sustained provider outage cannot let one
# turn loop until the round deadline.
MAX_GENERATION_FAILURES = 2

# Grace added to the turn clock for an in-flight generation, so a request that
# started with time on the clock is not cut off at the exact instant the turn
# expires. The round-level ceiling in game.start_games is the real backstop.
IMAGE_GRACE_SECONDS = 15.0


# Strong references to fire-and-forget tasks. asyncio only holds a WEAK
# reference to a running task, so a bare create_task() whose handle is dropped
...
async def _close_quietly(socket, code):
    """Close a socket we have already given up on, ignoring any error."""
    try:
...
class Team:
    ...
    async def safe_send(self, member_id, socket, payload):
        ...
        try:
            ...
            now = time.time()
            prev_count, prev_at = self._send_failures.get(member_id, (0, 0.0))
            # Failures older than the decay window are not part of this streak.
            if now - prev_at > SEND_FAILURE_DECAY_SECONDS:
                prev_count = 0
            failures = prev_count + 1
            self._send_failures[member_id] = (failures, now)
            logger.info(
                "Send to member %s failed (%s/%s within %.0fs): %s",
                member_id, failures, MAX_CONSECUTIVE_SEND_FAILURES,
                SEND_FAILURE_DECAY_SECONDS, exc,
            )
            if failures >= MAX_CONSECUTIVE_SEND_FAILURES:
                ...
                # Tell the client, or it never finds out.
                #
                # Dropping the socket server-side leaves the browser holding a
                # connection it still believes is healthy: the frontend only
                # re-authenticates on `onclose`, and it has no heartbeat, so
                # without this the player sits on a live-looking page receiving
                # nothing for the rest of the event while their team is
                # penalised every remaining turn. Closing it puts them straight
                # into the reconnect path.
                if socket is not None:
                    _fire_and_forget(_close_quietly(socket, code=4002))
            return False
    ...
    async def _broadcast_turn(self, active_uid):
        # Concurrent, like announce_to_team above -- turn_end_time is already
        # running by the time this is called, so a sequential loop spends the
        # ACTIVE player's clock waiting on their teammates: each safe_send can
        # block for SEND_TIMEOUT_SECONDS, so three congested sockets cost 4.5s,
        # and if the active player happens to be last in the dict they lose all
        # of it before they even see the image.
        tasks = [
            self.safe_send(member_uid, socket, {
                JSONFields.TYPE: Responses.GAMEPLAY_RESPONSE,
                JSONFields.MESSAGE: GamePlay.IMAGE_IN,
                JSONFields.IS_PLAYER_TURN: member_uid == active_uid,
                JSONFields.IMAGE: (
                    self.current_image if member_uid == active_uid else WAIT_YOUR_TURN_IMAGE
                ),
    ...
    async def run_round(self, round_num, time_per_round, timeout, penalty):
        ...
        try:
            ...
            # Record the zero before re-raising, or self.score ends up SHORTER
            # than the number of rounds played while game._play_all_rounds still
            # writes 0.0 for this team into round_data and the checkpoint. The
            # ROUND_SCORES list sent to this team's clients would then be
            # misaligned with the round numbers for the rest of the event.
            #
            # Latent until something actually cancels a round -- which the
            # round-level asyncio.wait_for in game.start_games now does by
            # design, so this is load-bearing.
            self.score.append(clamp_score(0.0))
            raise
        except Exception:
    ...
    async def _run_round(self, round_num, time_per_round, timeout, penalty):
        ...
        for index, uid in enumerate(self.rotation_order):
            ...
            # Failed generations for this turn, bounded by MAX_GENERATION_FAILURES.
            ...
            while True:
                ...
                if status == GamePlay.PROMPTED:
                    ...
                    if classify_prompt(prompt=current_prompt):
                        # Bounded by the turn clock. Without this the await
                        # escapes the turn deadline entirely -- it is checked
                        # above, before the call, and never again -- so one
                        # player behind a degraded provider could hold the
                        # round open past its clock. The round-level ceiling in
                        # game.start_games is the outer backstop; this is the
                        # one that keeps the turn honest.
                        budget = max(
                            5.0, (self.turn_end_time - time.time()) + IMAGE_GRACE_SECONDS
                        )
                        try:
                            generated = await asyncio.wait_for(
                                get_image(prompt=current_prompt), timeout=budget
                            )
                        except (asyncio.TimeoutError, TimeoutError):
                            logger.warning(
                                "Team %s: image generation exceeded %.1fs for player %s.",
                                self.id, budget, uid,
                            )
                            generated = None
                        ...
                        if generated is None:
                            ...
                            # Let them try again while there is time and budget
                            # left. A transient upstream error should not cost a
                            # player their turn with 60s still on the clock --
                            # during a 30s breaker-open window that would take
                            # out an entire cohort at once. Bounded by
                            # MAX_GENERATION_FAILURES so a sustained outage
                            # cannot loop the turn instead.
                            #
                            # Deliberately does NOT set prompt_submitted or send
                            # RECEIVED: the frontend locks its editor on either,
                            # and the player needs it back to retry.
                            if (
                                generation_failures < MAX_GENERATION_FAILURES
                                and time.time() < self.turn_end_time
                            ):
                                logger.info(
                                    "Team %s: generation failed for player %s (%s/%s); "
                                    "letting them resubmit.",
                                    self.id, uid, generation_failures, MAX_GENERATION_FAILURES,
                                )
                                await self.send_to(uid, {
                                    JSONFields.TYPE: Responses.ERROR_RESPONSE,
                                    JSONFields.MESSAGE: (
                                        "Image generation failed. Try submitting again."
                                    ),
                                })
                                continue

                            # Out of retries or out of clock. Keep the previous
                            # image in the chain rather than replacing it with a
                            # placeholder that the next player would then be
                            # asked to describe -- that used to corrupt the rest
                            # of the round for the whole team over one hiccup.
                            logger.warning(
